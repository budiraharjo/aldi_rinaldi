...
if (empty($_GET["ubah"])){
	...
	$kdt=$_GET["ubah"];
	$q=mysqli_query($connection,"select * from rute where kd_rute='$kdt'");
            ...
            <div class="panel panel-default">
                <div class="panel-heading"><i class="fa fa-plus fa-fw"></i> Isi Form ini dengan Lengkap</div>
                    <div class="panel-body">
						<form action="proses/ubah_rute.php?kdt=<?php echo $d["kd_rute"]?>" method="post">
							<div class="form-group">
							  <div class="row">
							    <div class="col-md-6">
								  <label>Kode rute<small><i>(* Isi dg 3 kode)</i></small></label>
								  <input type="text" name="kdt" maxlength="3" class="form-control" value="<?php echo $d["kd_rute"]?>" required placeholder="Kode rute..">
								...
								<div class="col-md-6">
								  ...
								  <input type="text" name="nama" class="form-control" value="<?php echo $d["rute"]?>" required placeholder="Nama rute..">
								  ...
								  <select name="kdkendaraan" class="form-control" required>
									...
									<option value="<?php echo $data["kd_kendaraan"]?>" <?php if ($d["kd_kendaraan"]==$data["kd_kendaraan"]){echo "selected";}?>><?php echo $data["kd_kendaraan"].' - '.$data["warna"]?></option>
									<?php }?>
								  </select>
								</div>
							  </div>
